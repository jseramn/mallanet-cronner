...
export default async function GalaxyPage() {
    ...
    return (
      <main className="flex flex-col items-center justify-center gap-4 py-24 text-center px-4">
        ...
        <p className="max-w-md text-sm text-muted-foreground text-pretty">
          La vista Galaxia muestra a tu equipo orbitando según su distancia horaria contigo.
  ...
  const windowStart = new Date(Date.now() - 2 * 24 * 3600_000).toISOString()
  const windowEnd = new Date(Date.now() + 2 * 24 * 3600_000).toISOString()
  ...
  return (
    <main className="flex flex-col gap-6 p-4 md:p-8 pb-24 md:pb-8">
      <header className="flex flex-col gap-1">
        <h1 className="text-xl font-semibold tracking-tight">Galaxia del equipo</h1>
        <p className="text-sm text-muted-foreground">
          {teamData.team.name} · vista orbital de disponibilidad en tiempo real
        </p>
      </header>
